...
THEME_COLOR = "#2e7d32" 
THEME_LIGHT = "#e8f5e9"
RELATION_DIR = "data/relations" 
TEMP_GRAPH_FILE = "data/temp_graph.html" 
...
def edit_avatar_dialog(char_id, current_avatar, char_name, current_book_id):
    ...
    with col_prev:
        if current_avatar:
            if current_avatar.startswith("http") or current_avatar.startswith("data:"):
                st.image(current_avatar, width=110)
            elif os.path.exists(current_avatar):
                st.image(current_avatar, width=110)
            else:
                st.info("无图")
        else:
...
def save_relations_to_disk(book_id, relations_data):
    if not os.path.exists(RELATION_DIR): os.makedirs(RELATION_DIR)
...
def ai_extract_characters(engine, db_mgr, current_book, current_book_id, progress_callback=None):
    ...
    [
        {{
            "name": "韩立", "gender": "男", "race": "人族", "role": "主角",
            "desc": "皮肤黝黑，相貌普通...", "is_major": true, "avatar": "" 
        }}
    ]
    ...
    try:
        ...
        for c in char_list:
            if c['name'] not in existing_names:
                ...
                if c.get('is_major') and (not avatar_url): 
                     avatar_url = generate_pollinations_url(c.get('desc'), c.get('name'))
                if not avatar_url:
                    seed = c['name']
                    avatar_url = f"https://api.dicebear.com/9.x/adventurer/svg?seed={seed}&flip=true"
                c['avatar'] = avatar_url
                final_list.append(c)
...
# --------------------------------------------------------------------------
# 核心渲染逻辑：手动构建 HTML (3D徽章 + 完美Tooltip)
# --------------------------------------------------------------------------
...
def generate_graph_html(nodes, edges, height="600px"):
    ...
    <script type="text/javascript">
        ...
        network.on("afterDrawing", function() {{
            if (this.tooltipsInjected) return;
            this.tooltipsInjected = true;
            setTimeout(function() {{
                var map = {{
                    'vis-zoomExtends': '全屏复位',
                    'vis-zoomIn': '放大', 'vis-zoomOut': '缩小',
                    'vis-up': '上移', 'vis-down': '下移',
                    'vis-left': '左移', 'vis-right': '右移'
                }};
                for (var cls in map) {{
                    var el = document.querySelector('.' + cls);
                    if (el) el.setAttribute('title', map[cls]);
                }}
            }}, 500);
        }});
...
def render_characters(engine, current_book_arg=None):
    ...
    with col_graph:
        ...
        if not chars_graph_rows:
            ...
            has_cache = saved_relations is not None
            ...
            try:
                ...
                for char in chars_graph:
                    ...
                    nodes.append({
                        ...
                        "borderWidth": 4, 
                        "borderWidthSelected": 6,
                        "color": { "border": color, "background": "#ffffff" }
                    })
    ...
    with col_edit:
        ...
        with tab_add:
            ...
            desc = st.text_area("描述", height=250, key="manual_desc")
            ...
            if st.button("确认添加", type="primary", use_container_width=True):
                if name:
                    ...
                    if up_new_add:
                         ...
                         if saved_path: final_av = saved_path
                    
                    if not final_av:
                         final_av = generate_bing_search_image(f"{current_book['title']} {name} 插画")
                         if not final_av:
                            final_av = f"https://api.dicebear.com/9.x/adventurer/svg?seed={name}&flip=true"
                    ...
                    if rel_target_name != "(无)" and rel_desc:
                        ...
                        log_operation('角色', f'添加关联: {name} -> {rel_target_name}')
        ...
        with tab_rels:
            ...
            if src_name and tgt_name and src_name != tgt_name:
                ...
                if existing_rel:
                    with c_act_1:
                        if st.button("更新关系", type="primary", use_container_width=True):
                            ...
                            st.rerun()
                    with c_act_2:
                        if st.button("删除连线", type="secondary", use_container_width=True):
                    ...
                    if st.button("➕ 建立新关系", type="primary", use_container_width=True):
                        new_rel = {
                            "source": src_id,
                            "target": tgt_id,
                            "label": rel_label,
            ...
            if not rows:
                ...
                for ch in all_chars_res:
                    with st.expander(f"{ch['name']} ({ch['role']})"):
                        ...
                        # 全宽描述 (移动到 col_edit_2 外面，消除左侧空白)
                        d_new = st.text_area("描述", ch.get('desc', ''), height=200, key=f"{kp}_d")

                        c_del, c_save = st.columns([1, 1])
                        if c_del.button("删除", key=f"del_{ch['id']}", type="secondary", use_container_width=True):
                             db_mgr.execute("DELETE FROM characters WHERE id=?", (ch['id'],))
                             update_book_timestamp_by_book_id(current_book_id)
                             log_operation('角色', f'删除角色：{ch["name"]}')
                             st.rerun()
                             
                        if c_save.button("保存", key=f"save_{ch['id']}", type="primary", use_container_width=True):
                             if n_new != ch['name'] or d_new != ch.get('desc'):
                                 db_mgr.execute("UPDATE characters SET name=?, desc=? WHERE id=?", (n_new, d_new, ch['id']))
                                 update_book_timestamp_by_book_id(current_book_id)
                                 log_operation('角色', f'编辑角色：{n_new}')
                                 st.rerun()
        ...
        with tab_ai:
            if st.button("🚀 AI 分析本书角色", type="primary", use_container_width=True):
                ...
                if ok:
                    ...
                    st.success(f"发现 {len(result)} 个角色")
                    st.rerun()
                else:
                    st.error(result)
            ...
            if extracted_data:
                if st.button("📥 全部添加", use_container_width=True):
                    for char_data in extracted_data:
                        try:
                            db_mgr.execute(
                                "INSERT INTO characters (book_id, name, role, gender, race, desc, is_major, avatar) VALUES (?,?,?,?,?,?,?,?)",
                                (current_book_id, char_data['name'], char_data.get('role', '路人'), char_data.get('gender', '未知'), char_data.get('race', '人族'), char_data.get('desc', ''), char_data.get('is_major', False), char_data.get('avatar', ''))
                            )
                        except: pass
                ...
                for idx, char_data in enumerate(extracted_data):
                    st.info(f"{char_data['name']} - {char_data.get('role')}")